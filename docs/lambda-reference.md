...
| `ORDER_EXECUTOR_FUNCTION` | order-executor Lambda関数名 (position-monitor用) |
...
5. **通貨毎にBUY/SELL/HOLD判定（ポジション非依存）**:
   ...
   - それ以外 → HOLD
6. BUY/SELLがある場合のみ、DynamoDB signals テーブルに保存済み
   - order-executor が EventBridge 15分毎起動で読み取り
...
EventBridge定期起動でDynamoDB signalsテーブルから最新シグナルを読み取り、Coincheck APIで成行注文を実行。
position-monitorからのSL/TP直接売り指示にも対応。

| 項目 | 値 |
|---|---|
| トリガー | EventBridge (15分間隔) / Lambda直接起動 (SL/TP) |
| メモリ | 256MB |
| タイムアウト | 60秒 |
...
aggregatorがDynamoDB signalsテーブルに全通貨のBUY/SELL/HOLD判定を保存。order-executorは15分毎に最新シグナルを読み取り、ポジション・残高を確認して実際の注文を実行する。

**処理順序**:
1. **SELL先**: 全対象のポジションがあれば売却、なければスキップ（資金確保）
2. **BUY**: 最もスコアの高い1通貨のみ購入（集中投資）

**BUYが複数ある場合**:
- 最もスコアの高い1通貨のみ購入（その他はスキップ）
- Kelly/フォールバック比率で投資額算出
...
6. 売り指示は Lambda 直接起動で order-executor に送信（即時実行）
...
flowchart TD
    ...
    subgraph 注文実行
        AGG -->|"保存"| DB_SIG
        DB_SIG -->|"EventBridge 15分毎"| OE["order-executor"]
        PM -->|"Lambda直接起動"| OE
