...
## 13. 02/12 テクニカルスコア計算の過剰反応修正（RSI/BB/SMA）

### 概要

BTC が RSI 67.6、レンジ相場でテクニカルスコア -0.71 と過剰に弱気な値が出た。
3つの指標のスコアリングが過剰に反応していた。

### 原因と修正

| 指標 | 旧ロジック | 問題 | 新ロジック |
|------|-----------|------|--------|
| RSI 30-70帯 | 単一線形式 `(50-rsi)/200` | 60-70の穏やかな過熱でもそれなりのペナルティ | 4ゾーン制: 40-60はデッドバンド(≈0)、30-40/60-70は最大30%のグラデーション |
| SMAクロス | バイナリ ±w_sma | 0.1%の微小な乖離でもフルペナルティ | 乖離率グラデーション (±2%でフルスコア) |
| BBポジション | 線形カーブ `(0.5-pos)*2*w` | 中央付近でも線形にスコアが動く | 二乗カーブ `(deviation/0.5)^2` → 中央は微小、端のみ急激 |

### スコア影響の比較 (BTC: RSI=67.6, BB=0.85, SMA乖離-0.3%, レンジ相場)

| 指標 | 旧スコア | 新スコア | 変化 |
|------|---------|---------|------|
| RSI 67.6 | -0.123 | -0.080 | 35%緩和 |
| SMA (0.3%乖離) | -0.150 | -0.023 | 85%緩和 |
| BB (0.85) | -0.245 | -0.172 | 30%緩和 |
| **テクニカル合計** | **-0.498** | **-0.254** | **49%緩和** |

### 変更箇所

| ファイル | 変更内容 |
|----------|----------|
| `services/technical/handler.py` | `calculate_score()`: RSI 4ゾーン制、SMA乖離率グラデーション、BB二乗カーブ |
