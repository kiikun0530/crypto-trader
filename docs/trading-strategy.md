...
| 分析 | 重み | スコア範囲 | データソース |
|---|---|---|---|
| テクニカル | **45%** | -1 〜 +1 | Binance 5分足OHLCV |
| AI予測 (Chronos) | **25%** | -1 〜 +1 | Binance 価格履歴 (Typical Price) |
| センチメント | 15% | -1 〜 +1 | CryptoPanic ニュース |
| マーケットコンテキスト | **15%** | -1 〜 +1 | F&G, Binance Funding, CoinGecko |

```
total_score = technical × 0.45 + chronos × 0.25 + sentiment × 0.15 + market_context × 0.15
```

> **ウェイト根拠 (Phase 3)**: Phase 2 の 3成分体制 (Tech=0.55, Chronos=0.30, Sent=0.15) では
> テクニカルが支配的すぎた。Phase 2 初日の3トレードは全てTechのみ正、Chronos/Sentが反対でも
> BUY発動 → 全敗。4成分に分散し、マクロ市場環境も加味した判定に改善。
...
buy_threshold  = BASE_BUY(0.28) × vol_ratio
sell_threshold = BASE_SELL(-0.15) × vol_ratio
...
- **平均時（BB幅 ≈ 3%）**: 基準閾値をそのまま使用（BUY +0.28 / SELL -0.15）

> **Phase 3 閾値調整根拠**: 4成分化でスコアが約15%圧縮 + Market Context の上方バイアス (+0.02)。
> 1,578件のシグナルデータで検証し、旧3成分と同等のシグナル頻度を維持する閾値に調整。
...
BUY閾値: +0.280 / SELL閾値: -0.150 | 重み: Tech=0.45 AI=0.25 Sent=0.15 Mkt=0.15
...
売りシグナルには `total_score <= sell_threshold (基準: -0.15)` が必要。典型的なシナリオ:
