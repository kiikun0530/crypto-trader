...
buy_threshold  = min(BASE_BUY(0.18) × vol_ratio + fng_adder, 0.45)  ← 上限キャップ
sell_threshold = BASE_SELL(-0.10) × vol_ratio
...
**ボラティリティ適応の意味**:
- **高ボラ時（BB幅 > 3%）**: 閾値を厳しく → ノイズに反応しない（例: BB幅6% → BUY閾値 +0.36）
- **低ボラ時（BB幅 < 3%）**: 閾値を緩く → 小さな確実なシグナルを拾う（例: BB幅2% → BUY閾値 +0.12）
- **平均時（BB幅 ≈ 3%）**: 基準閾値をそのまま使用（BUY +0.18 / SELL -0.10）
...
> スコア分布が約10%圧縮するため、BUY/SELL閾値を緩和してシグナル頻度を維持。
> 旧 BUY=0.28 / SELL=-0.15 → Phase 4 BUY=0.25 / SELL=-0.13。
>
> **Phase 5 閾値緩和根拠**: Phase 4の閾値(BUY=0.25)では4成分×4TFの同方向合意が要求されすぎ、
> 実運用で継続的にHOLDのみが発生。パフォーマンス実績データが蓄積されず、
> サービスとしての価値証明が不可能だった。
> BUY=0.18 / SELL=-0.10 に変更。これでもTech(+0.3)+Chronos(+0.2)+Sent(+0.1)+MKT(+0.1)=0.23
> のように少なくとも2主要成分がポジティブでないとBUYは発生しない。
> ボラ補正+F&G補正で実効閾値はさらに上がるため、安全マージンは十分。
...
result-checker Lambda が15分間隔で全通貨のBUY/SELLシグナルをチェックし、結果を記録。

| 時間窓 | 秒 | 意味 |
|---|---|---|
| result_1h | 3,600 | 1時間後の価格変動 |
| result_4h | 14,400 | 4時間後の価格変動 |
| result_12h | 43,200 | 12時間後の価格変動 |
| result_3d | 259,200 | 3日後の価格変動 |

**的中判定基準** (WIN_THRESHOLD_PCT = 0.3%):
...
| SELL | WIN | 価格変動 < -0.3% |
| SELL | LOSS | 価格変動 > +0.3% |
| SELL | DRAW | 変動が ±0.3% 以内 |

各時間窓の結果は signals テーブルの既存レコードに追記される。
各時間窓には `outcome`, `price_change_pct`, `exit_price`, `max_favorable_pct`, `max_adverse_pct` が含まれる。

### HOLD シグナルの正確性判定

HOLDが続く時期でもパフォーマンスデータを蓄積するため、  
HOLDシグナルの「動かなかったから正解」を評価。

| 時間窓 | 秒 | 意味 |
|---|---|---|
| hold_result_4h | 14,400 | 4時間後の価格変動 |
| hold_result_12h | 43,200 | 12時間後の価格変動 |

**HOLD的中判定基準** (HOLD_CORRECT_PCT = 0.3%):
